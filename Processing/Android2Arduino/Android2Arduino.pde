...
void setup()
{
	...
	String portName =
	    Serial.list()[mySerialPortList]; // Take the port in which the Arduino is connected
	myPort =
	    new Serial(this, portName, 9600); // Make the connection with this port through serial cable
	background(0);
}
...
void draw()
{
	...
	if (myServerRunning == true)
	{
		...
		if (thisClient != null)
		{
			...
			{                                      // If there is a connection do the following
				...
				text("" + thisClient.ip() + " turns it " + showOnScreen, 15,
				     textLine);           // Show the messages on the screen
	...
	{
		text("Server Stopped", 15,
		     textLine); // If the sever is stoped show the nessesery message on the screen
	}
}
